...
fn main() {
    ...
    let pw = args[1].clone();
    ctx.set_config(config::Config::Addr, Some("dice@sommerfuchs.info"))
        .unwrap();
    ...
    let contact_id = Contact::create(&ctx, "Diceroller", "dice@sommerfuchs.info").unwrap();
    let chat_id = chat::create_by_contact_id(&ctx, contact_id).unwrap();
    chat::send_text_msg(&ctx, chat_id, "Welcome to the Deltachat RPG Bot. Talk to me by writing [d:help].".into()).unwrap();
